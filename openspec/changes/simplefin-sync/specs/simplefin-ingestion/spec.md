...
- **WHEN** `finance sync setup <setup-token-url>` is run with a valid setup token URL
- **THEN** the Access URL is printed to stdout with instructions to add it to `.env` as `SIMPLEFIN_ACCESS_URL`

#### Scenario: Invalid token URL
- **WHEN** `finance sync setup <url>` is run with an invalid or already-claimed token URL
